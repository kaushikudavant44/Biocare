...
<section id="registration" class="log_sec">
	...
	<hr> 
    	<form method="POST" action='submitDoctorRegProcess' enctype="multipart/form-data" onsubmit="return validateForm()" id="reset1" name="regForm">
        	<div class="row">
        		<div class="col-sm-12">
                	<div class="form-group">
                     <label for="exampleInputEmail1" class="reqd">First Name</label>
            		 <input type="text" required class="input-text" name="fName" id="fName" max="20" onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123) || (event.charCode > 31 && event.charCode < 33)" data-rule-required="true"/>
            		 </div>
					</div>
					<div class="col-sm-12">
                	<div class="form-group">
                     <label for="exampleInputEmail1" class="">Middle Name</label>
            		 <input type="text" class="input-text" name="mName" id="mName" max="20" data-rule-required="true"/>
            		 </div>
					</div>
					<div class="col-sm-12">
                	<div class="form-group">
                     <label for="exampleInputEmail1" class="reqd">Last Name</label>
            		 <input type="text" required class="input-text" name="lName" id="lName" max="20" onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123) || (event.charCode > 31 && event.charCode < 33)" data-rule-required="true"/>
            		 </div>
					</div>
					<div class="col-sm-12">
                	<div class="form-group">
				...
				<div class="col-sm-6">
                	<div class="form-group">
            		 ...
            		 <input type="text" required class="input-text capitl_none" name="uname" id="uname" autocomplete="off" min="6" onkeypress="return (event.charCode > 0 && event.charCode < 32) || (event.charCode > 33 && event.charCode < 129)" max="20" data-rule-required="true"/>
	...
	<input type="password" required class="input-text capitl_none" name="password" id="password" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters and one special character" maxlength="20" required/><p id="validate1" style="color:red;position:absolute" ></p>
			    ...
			    <div class="form-group passwrd">
				    ...
				    <input type="password" required class="input-text capitl_none" name="confirm_password" id="confirm_password" /><p id='message' class="errormsg"></p>
             ...
             <!--    <hr>  -->
                ...
                <div class="form-group">
                    	...
                    	<!-- <input type="reset" value="Reset" onclick="resetFunction()" class="btn-fr-all"> -->
...
<div class="modal fade otp-pg" id="otp" tabindex="1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
       <!--  <button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick="timer(00)"><span aria-hidden="true">&times;</span></button> -->
...
$('#password, #confirm_password').on('keyup', function () {
	...
	if ($('#password').val() == $('#confirm_password').val()) {
	    ...
	    if($('#password').val().length>7)
	    	$('#submitButton').attr("disabled", false);
	    	    ...
	    	    $('#message').html('Password should be greater than 8').css({'line-height':'17px','font-size':'12px','color':'red'});
	    	    $('#submitButton').prop("disabled", "disabled");
	    	}
	    
	    
	  } else {
	    $('#message').html('Not Matching').css({'line-height':'17px','font-size':'12px','color':'red'});
	  $('#submitButton').prop("disabled", "disabled");
}
	...
	/* $('#password').on('focusout', function () {
		...
		if(!re.test($('#password').val()))
		    	...
		    	{
			$('#validate1').html('password should contain atleast one number,one special character and minimum 8 character').css('color', 'red');
	 ...
	 $.getJSON('${usernameValidationOfDoctor}',
				...
				function(data) {
					...
					if(data.message=='Success')   
						...
						//document.getElementById("validate").innerHTML="UserName Already Exists" ; 
						 $('#userNameMessage').html('UserName Already Exists').css({'line-height':'17px','font-size':'12px !important','color':'red'});
					...
					{
						 $('#userNameMessage').html('Username Available').css({'line-height':'17px','font-size':'12px !important','color':'green'});
